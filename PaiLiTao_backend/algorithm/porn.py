...
class Nude(object):
    ...
    def inspect(self):
        ...
        tkinter.messagebox.askokcancel('报告',"{_image}:\n result={_result}\n message='{_message}'".format(_image=_image, _result=self.result, _message=self.message))#弹框
        return "{_image}: result={_result} message='{_message}'".format(_image=_image, _result=self.result, _message=self.message)

    # 将在源文件目录【生成图片文件】，将皮肤区域可视化
...
def main(fname):
    #fname=r'E:\1projects\毕业设计-嗅图狗\pictures\局部识图测试\0.jpg'
    if os.path.isfile(fname):
        n = Nude(fname)
        n.resize(maxheight=800, maxwidth=600)
        n.parse()
        #n.showSkinRegions()#这是保存生成处理后的图片的
        print(n.result, n.inspect())
    else:
        print(fname, "is not a file")

# main(r'E:\1projects\毕业设计-嗅图狗\pictures\局部识图测试\5.jpg')
