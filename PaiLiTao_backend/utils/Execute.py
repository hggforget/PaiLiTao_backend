...
def startSearch01(var1, var2, var3):
    count1 = 0
    path = var2
    print(var1)
    im1 = Image.open(var1)
    imgs=list()
    length = str(len(os.listdir(path)))  # 显示总文件数量
    ...
    for (root, dirs, files) in os.walk(path):
        for filename in files:
            if os.path.splitext(filename)[1] in Const_Image_Format:  # 筛选格式
                ...
                j = pingheng * ((sizePingheng - 1) ** 2 - _dhash.classfiy_dHash(im1, im2,
                                                                                size=(sizePingheng, sizePingheng - 1)))
...
def _S_Repeat(var2, tree, h, root1):
        ...
        if list2[x] == -1:  # 如果list2的x1位置没有一样的，开始
            for y in range(len(list1)):
                ...
                if list2[y] == -1:  # 如果list2的y1位置还没有一样的，继续
                    k = 100 - 2.38 * _dhash.compCode(list1[x][0],
                                                     list1[y][0])  # 比对list1里的元组的[0]的哈希值【y去找x，找到y就属于x组】~~~~~~~~~~~
                    if (k >= 90):
                        list2[y] = x  # 组名均设为X的位置
                        list4[y] = k
                    else:
    ...
    for item_textS in listS2:
        tree.insert("", END,
                    values=[item_textS[0], item_textS[1], item_textS[2], item_textS[3], item_textS[4], item_textS[5],
                            item_textS[6], "组" + item_textS[7]])
